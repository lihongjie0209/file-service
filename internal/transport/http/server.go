...
import (
	...
	docs "github.com/lihongjie0209/file-service/docs"
	...
	"github.com/lihongjie0209/file-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, limiter *ratelimit.Limiter, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/files/uploads/initiate", handler.InitiateUpload)
	api.POST("/files/uploads/multipart/initiate", handler.InitiateMultipartUpload)
	api.POST("/files/uploads/multipart/authorize-part", handler.AuthorizeUploadPart)
	api.POST("/files/uploads/multipart/complete", handler.CompleteMultipartUpload)
	api.POST("/files/uploads/multipart/abort", handler.AbortMultipartUpload)
