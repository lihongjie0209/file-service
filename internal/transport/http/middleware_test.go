...
func TestIdempotencyExecutionCompletesAndReplaysFileDelete(t *testing.T) {
	t.Parallel()
	...
	router.Use(RequestID(), func(c *gin.Context) {
		...
		c.Next()
	}, IdempotencyExecution(manager, []string{"/api/v1/files/delete"}, slog.New(slog.NewTextHandler(io.Discard, nil))))
	router.POST("/api/v1/files/delete", func(c *gin.Context) { calls++; OK(c, gin.H{"id": "file-1"}) })
	out := httptest.NewRecorder()
	router.ServeHTTP(out, httptest.NewRequest(http.MethodPost, "/api/v1/files/delete", strings.NewReader(`{"id":"file-1"}`)))
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/files/delete", strings.NewReader(`{"id":"file-1"}`))
...
func TestIdempotencyExecutionBypassesFilePresignedURLsAndReads(t *testing.T) {
	t.Parallel()
	for _, route := range []string{"/api/v1/files/uploads/initiate", "/api/v1/files/uploads/multipart/initiate", "/api/v1/files/uploads/multipart/authorize-part", "/api/v1/files/downloads/authorize", "/api/v1/files/metadata/get", "/api/v1/files/metadata/list"} {
