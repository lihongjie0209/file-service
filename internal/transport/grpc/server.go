...
import (
	...
	"github.com/lihongjie0209/file-service/internal/environment"
	filedomain "github.com/lihongjie0209/file-service/internal/file"
	apphealth "github.com/lihongjie0209/file-service/internal/health"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, authService *auth.Service, authorizer platformauthz.Authorizer, healthService *apphealth.Service, fileService *filedomain.Service, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, logger *slog.Logger) (*Server, error) {
	...
	filev1.RegisterFileServiceServer(grpcServer, &fileServer{service: fileService})
...
func authenticateGRPC(ctx context.Context, method string, service *auth.Service, cfg config.Auth) (context.Context, error) {
	...
	if cfg.PSK.Enabled && auth.MatchesAny(method, cfg.PSK.GRPCMethods) {
		...
		return platformprincipal.WithContext(ctx, platformprincipal.Principal{ID: "file-service:psk", Type: platformprincipal.TypeServiceAccount}), nil
	}
