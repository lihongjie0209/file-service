...
import (
	...
	appdb "github.com/lihongjie0209/file-service/internal/database"
...
type integrationStorage struct{ info objectstorage.ObjectInfo }

func (integrationStorage) PresignUpload(context.Context, string, string, int64, string) (*url.URL, error) {
	return url.Parse("https://storage/upload")
}
func (integrationStorage) PresignDownload(context.Context, string) (*url.URL, error) {
	return url.Parse("https://storage/download")
}
func (s integrationStorage) Stat(context.Context, string) (objectstorage.ObjectInfo, error) {
	return s.info, nil
}
